  ...
  <AppLayout title="Plans & Billing">
    ...
    <div v-if="showForm" class="card p-6 mb-6">
      <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="flex items-center gap-2 pt-6">
          <input v-model="form.includes_all_modules" type="checkbox" id="includes_all" class="rounded border-gray-300" />
          <label for="includes_all" class="text-sm text-gray-600">Semua modul published</label>
        </div>
        <div v-if="!form.includes_all_modules" class="md:col-span-2">
          <label class="text-sm text-gray-600">Modul Kurasi</label>
        ...
        <div class="md:col-span-2">
          <label class="text-sm text-gray-600">Fitur</label>
          <div class="flex flex-wrap gap-3 mt-2">
            <label v-for="f in allFeatures" :key="f" class="inline-flex items-center gap-2 text-sm">
              <input type="checkbox" :value="f" v-model="form.features" class="rounded border-gray-300" />
              {{ f }}
            </label>
          </div>
        </div>
        <div class="md:col-span-2 flex justify-end gap-2">
          <button type="button" @click="showForm = false" class="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50">Batal</button>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <div v-for="plan in plans" :key="plan.id" class="card p-6 flex flex-col">
        ...
        <div class="text-xs text-gray-500 mb-2">Maks. {{ plan.max_users }} users · {{ plan.includes_all_modules ? 'Semua modul published' : (plan.modules?.length ?? 0) + ' modul kurasi' }}</div>
        <div class="flex flex-wrap gap-1 mb-3">
          <span v-for="f in plan.features" :key="f" class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 border border-teal-200 dark:border-teal-800">{{ f }}</span>
        </div>
