...
<template>
    ...
    <AppLayout title="Plans & Billing">
        ...
        <div v-if="showForm" class="bg-white rounded-xl shadow-sm p-6 mb-6">
            <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input v-model="form.name" type="text" required class="mt-1 w-full rounded-lg border-gray-300 text-sm" />
                </div>
                <div>
                    <label class="text-sm text-gray-600">Harga (ribuan IDR / bulan)</label>
                    <input v-model.number="form.price_monthly" type="number" min="0" class="mt-1 w-full rounded-lg border-gray-300 text-sm" />
                </div>
                <div>
                    <label class="text-sm text-gray-600">Maks. Users</label>
                    <input v-model.number="form.max_users" type="number" min="1" class="mt-1 w-full rounded-lg border-gray-300 text-sm" />
                </div>
                <div class="md:col-span-2">
                    <label class="text-sm text-gray-600">Fitur (satu per baris)</label>
                    <textarea v-model="form.features" rows="3" class="mt-1 w-full rounded-lg border-gray-300 text-sm"></textarea>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div v-for="plan in plans" :key="plan.id" class="bg-white rounded-xl shadow-sm p-6 flex flex-col">
